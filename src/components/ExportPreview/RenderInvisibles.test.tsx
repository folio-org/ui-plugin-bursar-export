...
import HandleInvisible, { splitAndInsert } from './RenderInvisibles';
...
test.each([
  ...
  ['abc\r\ndef', 'abc<br>def'],
  ['a b\tc\rd\ne', 'a b\tcd<br>e'],
])('Rendering with text=%s without showInvisible gives inner HTML=%s', (text, innerHTML) => {
  const { container } = render(<HandleInvisible text={text} showInvisible={false} />);
  expect(
...
test.each([
  ...
  ['abc\r\ndef', 'abc\\r\\ndef'],
  ['a b\tc\rd\ne', 'a•b\\tc\\rd\\ne'],
])('Rendering with text=%s and showInvisible gives inner text=%s', (text, textContent) => {
