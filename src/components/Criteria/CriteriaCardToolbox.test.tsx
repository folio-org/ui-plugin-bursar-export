...
describe('Criteria card toolbox', () => {
  ...
  it.each([CriteriaGroupType.ALL_OF, CriteriaGroupType.ANY_OF, CriteriaGroupType.NONE_OF])(
    'Outer group type %s results in only add',
    (type) => {
      render(
        withIntlConfiguration(
          <Form<FormValues> mutators={{ ...arrayMutators }} onSubmit={jest.fn()} initialValues={{ criteria: { type } }}>
            {() => <CriteriaCardToolbox prefix="criteria." root alone />}
          </Form>,
        ),
      );
  ...
  it.each([CriteriaGroupType.ALL_OF, CriteriaGroupType.ANY_OF, CriteriaGroupType.NONE_OF])(
    'Inner group type %s results in add and delete',
    (type) => {
      render(
        withIntlConfiguration(
          <Form<FormValues> mutators={{ ...arrayMutators }} onSubmit={jest.fn()} initialValues={{ criteria: { type } }}>
            {() => <CriteriaCardToolbox prefix="criteria." root={false} alone={false} />}
          </Form>,
        ),
      );
  ...
  it.each([CriteriaTerminalType.AGE, CriteriaTerminalType.LOCATION])(
    ...
    (type) => {
      ...
      expect(screen.queryAllByRole('button')).toHaveLength(1);
      expect(screen.getByRole('button', { name: 'trash' })).toBeVisible();
      expect(screen.getByRole('button', { name: 'trash' })).toHaveAttribute('disabled', '');
    },
  );
});
