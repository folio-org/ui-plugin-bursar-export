...
describe('DTO to header/footer conversion for initial values', () => {
  it.each<[BursarExportHeaderFooterTokenDTO, HeaderFooterToken]>([
    ...
    [{ type: 'Aggregate', value: 'NUM_ROWS', decimal: false }, { type: HeaderFooterTokenType.AGGREGATE_COUNT }],
    [
  ...
  ])('converts token %s to %s', (input, expected) => expect(dtoToHeaderFooterToken(input)).toEqual(expected));
